...
app.post('/form5', function(req, res) {
    ...
    //res.send("Firstame:"+ req.session.fname + "; Lastname:" +req.session.lname+ "; Languages:" +req.session.pval+"; days:"+req.session.dval+"; Email:"+req.session.eval);
    app.locals.fvar = "hidden";
    app.locals.lvar = "hidden";
    app.locals.pvar = "hidden";
    app.locals.dvar = "hidden";
...
app.post('/submit', function(req, res) {
    ...
    mongoDbObj.users.insert(userdata,{w:1},function(err, result){
        if(err){
            console.log(err);
        }
        else{
           console.log("success while inserting into database ");
           console.log(result);
